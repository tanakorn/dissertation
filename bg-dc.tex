...
\subsection{Local Concurrency vs Distributed Concurrency}

One core problem in any concurrent software systems is data races. A well-known
concurrency that causes data races is from thread execution interleaving in
multi-threaded software or what we call in this dissertation ``local
concurrency''. Multi-threaded software has become common in the age of
multiprocessor, but building multi-threaded software is hard.

However, in distributed systems, they are also subject to ``distributed
concurrency'' that is caused from interleaving of executions in multiple nodes.
Nodes in distributed systems do not have share memory and they access data in
other nodes by network messages, so basically, distributed concurrency is from
concurrent message arrivals and internal executions in running nodes.
