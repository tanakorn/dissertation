...
%\item \fev{Follower F crashed} in the past,
%\item \fev{F reboots} and joins the cluster,
%\item Leader L sync data with F and send snapshot, 
%\item \fev{In the middle of step 3-6}, client updates data with Tx-\#15; L forwards the update to F,
%\item F applies the update in memory only, due to a concurrent sync,
%\item L tells F syncing is finished, % \fev{after step 5},
%\item Client updates data with Tx-\#16; F writes update to disk correctly,
%\item \fev{F crashes},
%\item \fev{F reboots} and joins the cluster again,
%\item L sync data with F, but this time L sends only ``diff'' starting with Tx-\#17
%\item F permanently \fev{loses data} from Tx-\#15, inconsistent with L and other followers!
}
\end{quote}
\vspace{10pt}
\end{minipage}
