...
\newcommand{\ha}[1]{\jira{HADOOP}{HD}{#1}}
...
\def \scass {\ts{sck.cass}}
\def \sriak {\ts{sck.riak}}
\def \svold {\ts{sck.vold}}

\def \oon {$O(N)$\xspace} 
\def \oonn {$O(N^2)$\xspace} 
\def \oonnn {$O(N^3)$\xspace}

\def \sfind {\textsc{SFinder}\xspace}
\def \sfindp {\textsc{SFinder$_{PIL}$}\xspace}
\def \stest {\textsc{STest}\xspace}
\def \stestm {\textsc{STest$_{Mem}$}\xspace}
\def \stestp {\textsc{STest$_{PIL}$}\xspace}

\def \srep  {\textsc{SReplay}\xspace}

\def \numVersTotal {14\xspace}
\def \numVersOld {10\xspace}
\def \numVersNew {4\xspace}
\def \numVersCass {4\xspace}
\def \numVersHDFS {4\xspace}
...
\def \hdone {\hd{9198}\xspace}
\def \hdtwo {\ha{4061}\xspace}
\def \hdtri {\ha{1073}\xspace}
\def \hdfour {\hd{395}\xspace}
